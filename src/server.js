...
module.exports = async (client) => {
        ...
        if (banned) {
            client.users.fetch(req.user.id).then(async a => {
                client.channels.cache.get(channels.login).send(new Discord.MessageEmbed().setAuthor(a.username, a.avatarURL({
                    ...
                    dynamic: true
                })).setColor("RED").setDescription(`[**${a.username}**#${a.discriminator}](https://dbotlist.co/user/${a.id}) isimli kullanıcı **siteye** giriş yapmaya çalıştı fakat siteden engellendiği için giriş yapamadı.`).addField("Username", a.username).addField("User ID", a.id).addField("User Discriminator", a.discriminator))
            })
            req.session.destroy(() => {
                res.json({
                    login: false,
                    message: "You have been blocked from DBL.",
                    logout: true
                })
                req.logout();
            });
            ...
            client.users.fetch(req.user.id).then(async a => {
                client.channels.cache.get(channels.login).send(new Discord.MessageEmbed().setColor("PURPLE").setDescription(`**${a.username}#${a.discriminator}** just logged on to the website`))
    ...
    app.get("/robots.txt", function(req, res) {
        ...
        res.send(`Sitemap: https://dbotlist.co/sitemap.xml`);
    });

    app.get("/sitemap.xml", async function(req, res) {
        let link = "<url><loc>https://vdbotlist.co/</loc></url>";
        let botdataforxml = await botsdata.find()
        botdataforxml.forEach(bot => {
            link += "\n<url><loc>https://dbotlist.co/bot/" + bot.botID + "</loc></url>";
        })
...
.setColor("GREEN")
.setAuthor("Bot Added")
.setDescription(`<@${req.user.id}> added **${a.tag}**.`)
.addField("**Bot**", `[${a.username}](https://dbotlist.co/bot/:botID)`, true)
.addField("**User**", `[<@${req.user.id}>](https://dbotlist.co/user/:userID)`, true))
.setTimestamp();
    ...
    app.post("/bot/:botID/edit", checkMaintence, checkAuth, async (req, res) => {
        ...
        client.users.fetch(req.params.botID).then(a => {
            client.channels.cache.get(channels.botlog).send(new Discord.MessageEmbed().setColor("GREEN").setDescription(`<@${req.user.id}> edited **${a.tag}** `))       
    ...
    app.post("/certification/apply", checkMaintence, checkAuth, async (req, res) => {
        ...
        let botdata = await botsdata.findOne({
            botID: rBody['bot']
                  ...
                  client.channels.cache.get(channels.botlog).send(new Discord.MessageEmbed()
.setColor("GREEN")
.setDescription(`User **${req.user.username}** requested a certificate for her bot named **${botdata.username}**.`))
    });
    //
...
.setDescription(`<@${botdata.ownerID}>'s bot named **${bota.tag}** has been approved. `)
.addField("**Bot**", `[${bota.tag}](https://dbotlist.co/bot/:botID)`, true)
.addField("**User**", `[<@${botdata.ownerID}>](https://dbotlist.co/user/:userID)`, true)).setTimestamp();
        });
...
.setColor("RED")
.setAuthor("Bot Declined")
.setDescription(`<@${botdata.ownerID}>'s bot named **${botdata.username}** has been declined. `)
.addField("Reason", `${rBody['reason']}`, true))
.setTimestamp();
    ...
    app.get("/admin/certificate/give/:botID", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
        ...
        client.users.fetch(botdata.botID).then(bota => {
            client.channels.cache.get(channels.botlog).send(new Discord.MessageEmbed()
.setColor("GREEN")
.setAuthor("Bot Certificate")
.setDescription(`<@${botdata.ownerID}>'s bot named **${bota.tag}** has been granted a certificate. `)
.addField("**Bot**", `[${bota.tag}](https://dbotlist.co/bot/:botID)`, true)
.addField("**User**", `[<@${botdata.ownerID}>](https://dbotlist.co/user/:userID)`, true)).setTimestamp();
        });
    ...
    app.post("/admin/certificate/delete/:botID", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
        ...
        client.users.fetch(botdata.botID).then(bota => {
          ...
          .setColor("RED")
.setAuthor("Bot Declined")
.setDescription(`<<@${botdata.ownerID}>'s bot named **${bota.tag}** has not been granted a certificate. `)
.addField("Reason", `${rBody['reason']}`, true))
.setTimestamp();
        });
        await appsdata.deleteOne({
    ...
    app.post("/admin/addcode", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
        ...
        client.channels.cache.get(channels.codelog).send(new Discord.MessageEmbed()
            ...
            .setDescription(`The user named **[${req.user.username}](https://dbotlist.co/user/${req.user.id})** added the code named **${rBody['codename']}** to the system.`)
            .addField("Code Link", `https://dbotlist.co/code/${kod}`, true)
    ...
    app.post("/admin/editcode/:code", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
        ...
        client.channels.cache.get(channels.codelog).send(new Discord.MessageEmbed()
            ...
            .setDescription(`The user named **[${req.user.username}](https://dbotlist.co/user/${req.user.id})** edited the code named **${rBody['codename']}**.`)
            .addField("Code Link", `https://dbotlist.co/code/${kod}`, true)
            .addField("Code Description", rBody['codedesc'], true)
            .addField("Code Category", rBody['category'], true)
        )
    ...
    app.post("/admin/maintence", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
        ...
        client.channels.cache.get(channels.webstatus).send(`<a:maintence:833375738547535913> DBL has been switched to __maintance__ due to **${req.body.reason}**`).then(a => {
            new maintenceSchema({
                server: config.server.id,
    ...
    app.post("/admin/unmaintence", checkMaintence, checkAdmin, checkAuth, async (req, res) => {
        ...
        const bakimsonaerdikardesTopic = new dc.MessageEmbed()
            .setAuthor("DBL", client.user.avatarURL())
            .setThumbnail(client.user.avatarURL())
            .setColor("GREEN")
            .setDescription(`<a:online:833375738785824788> DBL are **active** again!\n[Click to redirect website](https://dbotlist.co)`)
            .setFooter("DBL © All rights reserved.");
        await client.channels.cache.get(channels.webstatus).messages.fetch(bakimdata.bakimmsg).then(a => {
            a.edit(`~~ <a:maintence:833375738547535913> DBL has been switched to __maintance__ due to **${bakimdata.reason}** ~~`, bakimsonaerdikardesTopic)
        })
